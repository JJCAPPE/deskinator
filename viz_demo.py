...
try:
    from planning.coverage import SimpleWallFollower, SimpleRectangleFit, CoveragePlanner
...
class SimulatedRobot:
    ...
    def __init__(self, start_pose: Tuple[float, float, float], table_bounds: Tuple[float, float, float, float]):
        """
    ...
    def read_sensors(self) -> Tuple[bool, bool]:
        ...
        left_on = (self.min_x <= left_x <= self.max_x and 
                   self.min_y <= left_y <= self.max_y)
        right_on = (self.min_x <= right_x <= self.max_x and 
                    self.min_y <= right_y <= self.max_y)
...
class TableSimulator:
    ...
    def __init__(self, center: Tuple[float, float], width: float, height: float, heading: float = 0.0):
        """
...
def simulate_wall_following():
    ...
    # Create table
    table = TableSimulator(center=(0.0, 0.0), width=1.5, height=1.0, heading=0.0)
    ...
    print(f"Starting pose: ({start_x:.2f}, {start_y:.2f}, {np.rad2deg(start_theta):.1f}°)")
    
    # Create robot
    robot = SimulatedRobot(start_pose, table.get_bounds())
    
    # Create wall follower
    wall_follower = SimpleWallFollower(forward_speed=0.1, turn_speed=0.5)
...
def simulate_coverage(robot, rect_fit, table):
    ...
    def pure_pursuit(pose, waypoint, lookahead=0.15):
        ...
        # Control
        v = 0.1 if dist > lookahead else 0.05
    ...
    try:
        while t < max_time and not planner.is_complete():
            ...
            if int(t / dt) % 5 == 0:
                ...
                bounds = (swept_map.min_x, swept_map.max_x, swept_map.min_y, swept_map.max_y)
        ...
        viz.update(
            ...
            rectangle=rectangle,
            coverage_grid=swept_map.get_grid(),
            swept_map_bounds=(swept_map.min_x, swept_map.max_x, swept_map.min_y, swept_map.max_y),
...
def main():
    ...
    robot, wall_follower, rect_fit, table = simulate_wall_following()
    
    # Phase 2: Coverage
    if rect_fit.get_rectangle():
        simulate_coverage(robot, rect_fit, table)
    else:
